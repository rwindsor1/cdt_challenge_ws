...
FOLLOWER_OUTPUT PositionController::computeControlCommand(Eigen::Isometry3d current_pose, int64_t current_utime){
  ...
  double alpha = 0.01/distance_to_goal;
  ...
  // Linear:
  std::cout<< "angular velocity "<< abs(angular_velocity) << std::endl;
 ...
 // linear_forward_y = 0.1;
  ...
  // set outputs
  output_linear_velocity_ = Eigen::Vector3d(speed, 0, 0);
